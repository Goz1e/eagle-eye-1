...
const COLORS = ['#2563eb', '#7c3aed', '#dc2626', '#059669', '#ea580c', '#be185d'];
...
export function WalletReportDashboard({ 
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'wallets' | 'analysis' | 'insights'>('overview');
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-xl font-semibold text-slate-900">Wallet Analysis Report</h1>
                <p className="text-sm text-slate-500">
                  Generated {reportData.generatedAt.toLocaleDateString()} • {reportData.analysisPeriod}
                </p>
              </div>
            </div>
                ...
                {summaryMetrics.map((metric, index) => (
                  <div key={index} className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                    ...
                    <div className="text-3xl font-bold text-slate-900 mb-2">
                      {metric.title.includes('Value') || metric.title.includes('savings') 
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={reportData.comparativeMetrics.volumeComparison.slice(0, 10)}>
                      ...
                      <Tooltip 
                        labelFormatter={(label) => `Wallet: ${truncateAddress(label)}`}
                        formatter={(value) => [formatUSD(value as number), 'Volume']}
                      />
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <PieChart>
                      <Pie
                        ...
                        dataKey="walletCount"
                      >
                        {reportData.comparativeMetrics.balanceDistribution.map((entry, index) => (
                          <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                        ))}
                      </Pie>
                      <Tooltip formatter={(value, name) => [value, 'Wallets']} />
                    </PieChart>
                  </ResponsiveContainer>
                </div>
              </div>
          ...
          {activeTab === 'wallets' && (
            <div className="space-y-6">
              ...
              {reportData.wallets.map((wallet, index) => (
                <div key={wallet.walletAddress} className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center space-x-3">
                    ...
                    <div className="flex items-center space-x-2">
                      <button className="text-slate-400 hover:text-slate-600">
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v8a2 2 0 01-2 2h-2m-6-4l2 2m0 0l2-2m-2 2V10" />
                        </svg>
                      </button>
                      <button className="text-slate-400 hover:text-slate-600">
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className="border-t border-slate-200 pt-4">
                    ...
                    <div className="text-sm text-slate-500">
                      Last activity: {wallet.lastUpdated 
          ...
          {activeTab === 'analysis' && (
            <div className="space-y-8">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                ...
                <ResponsiveContainer width="100%" height={400}>
                  <BarChart data={reportData.comparativeMetrics.gasEfficiencyComparison}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis 
                      dataKey="address" 
                      tickFormatter={truncateAddress}
                    ...
                    <Tooltip 
                      ...
                      formatter={(value, name) => [
                        name === 'efficiency' ? `${value}%` : formatUSD(value as number),
                        name === 'efficiency' ? 'Efficiency' : 'Potential Savings'
                      ]}
                    />
                    <Bar dataKey="efficiency" fill="#2563eb" name="Efficiency %" />
                    <Bar dataKey="potentialSavings" fill="#059669" name="Potential Savings" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
                      ...
                      <div className="flex-1">
                        ...
                        {insight.metric && insight.value && (
                          <div className="mt-3 text-sm text-slate-500">
                            <span className="font-medium">{insight.metric}:</span> {insight.value}
                          </div>
                        )}
                      </div>
                    </div>
            ...
            <div className="space-y-4">
              ...
              {selectedInsight.walletAddress && (
                <div className="bg-slate-50 rounded-lg p-4">
                  <h4 className="font-medium text-slate-900 mb-2">Related Wallet:</h4>
                  <p className="font-mono text-slate-800">{selectedInsight.walletAddress}</p>
                </div>
              )}
            </div>
          </div>
