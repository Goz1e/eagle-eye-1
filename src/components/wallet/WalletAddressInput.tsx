...
export default function WalletAddressInput({
  ...
  const parseAndValidateAddresses = useCallback(
    (text: string): AddressState[] => {
      const lines = text
        .split(/[\n,]/)
        .map((line) => line.trim())
        .filter(Boolean);
  ...
  const confirmAllValid = useCallback(() => {
    ...
    const confirmedAddresses = validAddresses.map((addr) => ({
      ...addr,
      confirmed: true,
    }));
  ...
  const validAddresses = useMemo(
    () => addresses.filter((addr) => addr.isValid),
    [addresses]
  );
  ...
  useEffect(() => {
    if (validAddresses.length > 0 && hideConfirmButton) {
      const validAddressStrings = validAddresses.map((addr) => addr.address);
      onAddressesConfirmed(validAddressStrings);
    }
  }, [validAddresses, hideConfirmButton, onAddressesConfirmed]);
            ...
            <div className="flex items-center space-x-2 text-xs">
              ...
              {hasErrors && (
                <span className="text-red-600">
                  ✗ {addresses.filter((addr) => !addr.isValid).length} errors
                </span>
              )}
            </div>
          </div>
      ...
      {!hideConfirmButton && (
        <div className="flex flex-col sm:flex-row items-center justify-between space-y-3 sm:space-y-0">
          <div className="flex items-center space-x-3">
            {canConfirm && (
              <button onClick={confirmAllValid} className="btn-primary text-sm">
                <svg
            ...
            <div className="flex items-center space-x-4">
              <div className="text-right">
                ...
                <div className="text-sm font-medium text-slate-700">
                  {validAddresses.length} valid address
