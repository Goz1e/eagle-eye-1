...
export function ProcessingDisplay({
  ...
  const percentage =
    totalWallets > 0 ? (progress.currentWallet / totalWallets) * 100 : 0;
  const processingTimeSeconds = Math.floor(progress.processingTime / 1000);
  const estimatedTimeRemaining =
    totalWallets > 0
      ? Math.ceil(
          ((progress.processingTime / progress.currentWallet) *
            (totalWallets - progress.currentWallet)) /
            1000
        )
      : 0;
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-8">
      ...
      <div className="mt-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
        ...
        <ul className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
          <li>
            • Analysis time depends on wallet activity and network conditions
          </li>
