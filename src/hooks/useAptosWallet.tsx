...
export default function useAptosWallet() {
  const useTransactions = () => {
    ...
    function parseTransactionData(
      ...
      for (const tx of transactions) {
        ...
        if (!to && tx.payload?.arguments?.[1]) {
          ...
          if (
            typeof recipientArg === "string" &&
            recipientArg.startsWith("0x")
          ) {
            to = recipientArg;
          }
        }
    ...
    function saveToCsv(data: CsvData[], filename: string) {
      ...
      console.log(
        `Successfully saved ${data.length} transactions to ${filename}`
      );
    }
    ...
    async function getLatestTransactions() {
      ...
      try {
        ...
        console.log(
          `Fetched ${allTransactions.length} raw transactions in total.`
        );
